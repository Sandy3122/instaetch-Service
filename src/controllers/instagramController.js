...
//         const cachedData = cacheManager.getCachedMediaInfo(shortcode);
//         if (cachedData) {
//           console.log(`✅ Cache hit for media: ${shortcode}`);
//           return this.formatMediaResponse(cachedData, url, shortcode, res);
//         }
//       }
//       console.log(`❌ Cache miss for media: ${shortcode}. Scraping...`);

//       const result = await this.scraper.getMediaInfo(url);

//       if (!result.success) {
//         return res.status(404).json({
//           error: 'Media not found or scraping failed',
//           details: result.error,
//         });
//       }

//       const mediaData = result.data; // { items: [...], is_carousel: ... }
...
//       console.log(`✅ Successfully formatted ${responses.length} media item(s)`);
//       res.json(responses);
//     } catch (error) {
