...
// Only Images from the carousel

/**
 * @fileoverview Instagram scraper utility using Playwright.
 * This file contains the InstagramScraper class which is responsible for
 * launching a browser, navigating to Instagram URLs, and extracting media.
 * This version uses highly specific selectors to get only the main slide content.
 */
...
//     /**
//      * Clicks through the entire carousel, extracting media URLs one by one
//      * to combat DOM virtualization and using precise selectors to get only slide content.
//      * @param {import('playwright').Page} page The Playwright page object.
//      * @returns {Promise<Array<{type: string, url: string}>>} A list of unique media items.
//      */
//     async clickAndScrapeCarousel(page) {
...
//         const scrapeCurrentlyVisible = async () => {
//             const currentMediaOnPage = await page.evaluate((selector) => {
//                 const results = [];
//                 const list = document.querySelector(selector);
//                 if (!list) return results;
...
//                 const listItems = list.querySelectorAll('li._acaz');
//                 listItems.forEach(item => {
//                     // This is the container for the actual slide media (image or video)
//                     const mediaContainer = item.querySelector('div._aagu');
//                     if (!mediaContainer) return; // If it's not a proper slide, skip it.

//                     // Check for a video inside the main container
//                     const video = mediaContainer.querySelector('video');
//                     if (video && video.src) {
//                         results.push({ type: 'video', url: video.src });
//                         return; // Found video, no need to check for image
//                     }

//                     // Check for an image inside the main container's specific image div (_aagv)
//                     // This is much more specific and avoids capturing thumbnails/avatars.
//                     const imgContainer = mediaContainer.querySelector('div._aagv');
//                     if(imgContainer) {
...
//             currentMediaOnPage.forEach(media => {
//                 if (media.url && !collectedMedia.has(media.url)) {
//                     console.log(`[+] Found new slide: ${media.type} at ${collectedMedia.size + 1}`);
//                     collectedMedia.set(media.url, media);
//                 }
//             });
//         };
...
//             } catch (error) {
//                 // This error is expected when the "Next" button is no longer visible
//                 console.log('✅ Reached the end of the carousel. Finalizing collection.');
//                 // One final scrape to catch the very last slide
//                 await scrapeCurrentlyVisible();
//                 break; // Exit the loop
...
//         console.log(`🖼️  Total unique slides found: ${collectedMedia.size}.`);
//         return Array.from(collectedMedia.values());
//     }
